...
def delay_sensitive_scheduler(users, resources_available):
    ...
    while sum(combined_weightings) > 0:
        ...
        # case: not enough time left to send packet, ignore
        if (lowest_times_to_timeout[highest_weight_id] == 1) and (
                units_allocated < resources_requested[highest_weight_id]):
            units_allocated = 0
...
def random_scheduler(users, resources_available):
    resource_allocation = np.zeros(len(users))
    ...
    while (sum(resources_requested) > 0) and (resources_available > 0):
        allocation_id = np.random.choice(range(len(users)))
        if resources_requested[allocation_id] > 0:
            resource_allocation[allocation_id] += 1
            resources_requested[allocation_id] -= 1
            resources_available -= 1
